...
bh=np.array([0.2e-6])                                   # Room temp resistance of the heater
...
R0=np.array([10,40,80])
# Cartesian product of input variables
idx = pd.MultiIndex.from_product([R0, ts,  frequency], names=["R0", 'ts', "frequency"])
# Reset the index of the DataFrame, and use the default one instead. If the DataFrame has a MultiIndex, this method can remove one or more levels.
df = pd.DataFrame(index=idx).reset_index()

omega = (bh[0]**2 * df["frequency"].values) * (2 * math.pi / D)
P=( V0**2/(2*df["R0"].values))        # Omega is vectorized naturally.
...
for R0_elem in R0:
    fname = f"R0={R0_elem:.4e}.csv"
    df_save = df[(df["R0"] == bh_elem)]
...
for i in range(2):
    ...
    for R0_array in R0:
        ...
        column = columns[i]
        axs[i].plot(
            df[(df["R0"] == R0_array) & (df["ts"] == ts[0])][column],
            df[(df["R0"] == R0_array) & (df["ts"] == ts[0])]['Re'],
            colours[np.where(R0 == R0_array)[0][0]],
            label='bh=' + '{0:.1f}'.format((R0_array/1e-6)) + '\u03BCm - ' + r't$_{\rm s}$' + ' = ' + '{0:.0f}'.format((ts[0]/1e-6)) + 'um')
        # Real part Vs Thermal penetration depth, bh = to bh[1]
        axs_top1.plot(
            df[(df["R0"] == R0_array) & (df["ts"] == ts[0])]['T_depth'],
            df[(df["R0"] == R0_array) & (df["ts"] == ts[0])]['Re'],
            colours[np.where(bh == bh_array)[0][0]],
            label='R0=' + '{0:.1f}'.format((R0_array/1e-6))   + '\u03BCm - ' + r't$_{\rm s}$' + ' = ' + '{0:.0f}'.format((ts[0]/1e-6)) + 'um')

        # Imagine part Vs Frequency, bh = to bh[1]
        axs[i].plot(
            df[(df["R0"] == R0_array) & (df["ts"] == ts[0])][column],
            df[(df["R0"] == R0_array) & (df["ts"] == ts[0])]['Im'],
            colours[np.where(R0 == R0_array)[0][0]],
            label='_nolegend_')
    ...
    axs[i].plot(
        df[(df["R0"] == R0[R0.size-1]) & (df["ts"] == ts[0])][column],
        df[(df["R0"] == R0[R0.size-1]) & (df["ts"] == ts[0])]['Asympt_Re'],
        color= 'black',
        linestyle='--',
        label='bh=' + '{0:.1f}'.format((R0[R0.size-1]/1e-6)) + '\u03BCm -  ' + r't$_{\rm s}$' + ' = ' + '{:.0f}'.format((ts[0]/1e-6)) + 'um  (asympt)')
    # Asympt Imagine part Vs Thermal penetration depth, bh = to bh[1], choose the larger number from the bh array
    axs[i].plot(
        df[(df["R0"] == R0[R0.size-1]) & (df["ts"] == ts[0])][column],
        df[(df["R0"] == R0[R0.size-1]) & (df["ts"] == ts[0])]['Asympt_Im'],
        color= 'black',
        linestyle='--',
        label='_nolegend_')
